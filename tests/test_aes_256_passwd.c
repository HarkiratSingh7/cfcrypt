...
int main()
{
    printf("Testing the 256 AES file encryption / decryption with password\n");

    /* Create a big file */
    const char *file_name = "TEST3_passwd.txt";
    const char *out_file = "TEST3_passwd.enc";
    const char *dec_file = "TEST3_passwd.dec";
    ...
    assert(SUCCESS == encrypt_aes_256_file(file_name, out_file, &key1, 0, 1));
    assert(SUCCESS == decrypt_aes_256_file_with_key(out_file, dec_file, &key2, 1));
